<template>
  <div>
    ...
    <center>
      <v-card class="cardShowuser mt-0" width="500">
        <v-card-title>
       ...
       <input type="submit" name="export" class="btn btn-success"  />
        ...
        <v-row justify="center" class="ma-5">
          <v-avatar color="#ffeee2">
            <span class="text-uppercase" style="font-size: 25px; color: #fcad74"
        ...
        <v-card-text>
          <div class="ma-10">
            ...
            <!-- <v-row
              ><span class="primary--text mr-3 font-weight-medium" >ชื่อ </span>
              {{ user.user_firstname }}</v-row
            >
            <v-row
              ><span class="primary--text mr-3 font-weight-medium"
                >นามสกุล
              </span>
              {{ user.user_lastname }}</v-row
            >
            <v-row
              ><span class="primary--text mr-3 font-weight-medium">อีเมล</span>
              {{ user.user_email }}</v-row
            >
            <v-row
              ><span class="primary--text mr-3 font-weight-medium"
                >วันเดือนปีเกิด
              </span>
              {{ user.user_birthday }}</v-row
            >
            <v-row
              ><span class="primary--text mr-3 font-weight-medium">อายุ </span>
              {{ user.user_age }}</v-row
            >
            <v-row
              ><span class="primary--text mr-3 font-weight-medium">อำเภอ </span>
              {{ user.user_district }}</v-row
            >
            <v-row
              ><span class="primary--text mr-3 font-weight-medium"
                >จังหวัด
              </span>
              {{ user.user_province }}</v-row
            >
            <v-row
              ><span class="primary--text mr-3 font-weight-medium"
                >กลุ่มผู้ใช้งาน</span
              >
              {{ user.user_type }}</v-row
            > -->
          </div>
        </v-card-text>
        <!-- <v-virtual-scroll
...
export default {
  ...
  data: () => ({
    ...
    birthdayRules: [(v) => !!v || 'กรุณากรอกวันเดือนปีเกิด'],
    districtRules: [(v) => !!v || 'กรุณากรอกอำเภอ'],
    provinceRules: [(v) => !!v || 'กรุณากรอกจังหวัด'],
    user: {
