  ...
  <div>
    ...
    <v-card class="mt-7 mb-7 pa-5">
      <h1 class="text-center ma-5">
        บทที่ {{ lesson_id }} <span class="teal--text">{{ lesson_name }}</span>
      </h1>
      ...
      <v-card
        elevation="0"
        style="border-color: #fcad74; border-width: 2px"
        rounded
        outlined
      >
        <v-card-title> เว็บไซต์ที่เกี่ยวข้อง </v-card-title>
        <v-card-subtitle>
...
export default {
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        {
          text: 'รายละเอียดบทเรียนของผู้ใช้งาน',
          disabled: true
        }
      ]
    }
  ...
  methods: {
    async getLesson () {
      const url =
        'http://localhost/vue-backend/lessonDetail.php?id=' + this.lesson_id
      // alert(url)
